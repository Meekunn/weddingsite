...
export const ClaimGoodiesDto = z.object({
    ...
    phoneNumber: z
        ...
        .min(14, { message: "Phone must be a valid Nigerian number" })
...
export const GoodiesIdDto = z.object({
    goodies_id: z
        .string({ required_error: "Goodies UUID is required" })
        .uuid({ message: "A valid goodies UUID is required" })
        .nonempty({ message: "Please enter a goodies UUID" })
});
export type TGoodiesIdDto = typeof GoodiesIdDto._type;

export const CreateBillPayloadDto = z.object({
    phoneNumber: z
        .string({ required_error: "Phone must be a valid Nigerian number" })
        .min(14, { message: "Phone must be a valid Nigerian number" })
        .nonempty({ message: "Phone number cannot be empty" }),
